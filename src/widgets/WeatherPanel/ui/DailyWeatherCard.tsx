...
export const DailyWeatherCard = () => {
  ...
  const locationName = selectedLocation?.placeName ?? "";

  const { data, error } = useQuery({
    queryKey: ["weather", selectedLocationId],
    queryFn: () => fetchWeather(selectedLocationId!),
    enabled: !!selectedLocationId,
  });

  useEffect(() => {
    console.log("📍 selectedLocationId 상태:", selectedLocationId);
  }, [selectedLocationId]);
    ...
    <div className="flex w-[1080px] p-8 flex-col items-start gap-3">
      ...
      <div className="flex flex-col items-center gap-[10px] p-[10px] self-stretch">
        <div className="flex justify-center items-center gap-[10px]">
          <div className="w-[160px] h-[160px] aspect-square flex justify-center items-center">
            <img
              src={getWeatherIcon(weatherDescription, isDaytime)}
              alt="weather icon"
        ...
        <div className="text-sm text-gray-600 font-[700] font-pretendard">
          체감 {formatTemperature(feelsLike)}° · 습도 {humidity}% ·{" "}
          {windDirection}풍 {windSpeed}m/s
        </div>
      </div>
      ...
      <div className="flex px-0 py-[12px] justify-center items-start gap-[16px] self-stretch rounded-[16px]">
        <div className="flex flex-col items-center gap-[10px] w-[120px] px-[24px] py-[12px] rounded-[12px] bg-[#CCE8FF]">
          <span className="text-sm font-semibold font-[500] text-black font-pretendard">
            미세먼지
          </span>
          <span className="text-lg font-bold text-black font-pretendard">
            {airPollution.pm10Level}
          </span>
        </div>

        <div className="flex flex-col items-center gap-[10px] w-[120px] px-[24px] py-[12px] rounded-[12px] bg-[#CEFFCC]">
          <span className="text-sm font-semibold text-black font-[500]font-pretendard">
            초미세먼지
          </span>
          <span className="text-lg font-bold text-black font-pretendard">
            {airPollution.pm2_5Level}
          </span>
        </div>
